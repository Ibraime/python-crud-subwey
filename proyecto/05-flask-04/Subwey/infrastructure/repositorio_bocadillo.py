...
class RepositorioBocadillo:
    ...
    def modificar(self, nombre_actual, nuevo_nombre,
                ingredientes, descuento=None, autor=None):
        """
        ...
        try:
            ...
            if nombre_actual != nuevo_nombre:
                if self.obtener_por_nombre(nuevo_nombre):
                    raise BocadilloDuplicadoError(
                        "Ya existe un bocadillo con ese nombre."
                    )
            ...
            """, (
                nuevo_nombre,
                autor.nombre if autor else boc.autor.nombre,
                1 if descuento is not None else 0,
                float(descuento) if descuento is not None else 0,
                nombre_actual
            ))
            ...
            conn.rollback()
            raise BocadilloDuplicadoError(
                "Error de integridad (FK o duplicado)."
            )
