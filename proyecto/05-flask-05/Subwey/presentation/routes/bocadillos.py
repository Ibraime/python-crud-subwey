...
def crear():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"Bocadillo {nombre} creado correctamente.", "exito")
        ...
        return render_template(
            ...
            ingredientes_actuales=ingredientes_sel,
            promo=bool(promo),
            descuento=descuento,
            error=error
        )
...
def editar(nombre):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"Bocadillo {nombre} modificado correctamente.", "exito")
        ...
        return render_template(
            ...
            nombre=nuevo_nombre,
            usuarios=repo_usuario.listar(),
            usuario_actual=bocadillo.autor.nombre,
            ingredientes=ingredientes,
            ingredientes_actuales=ingredientes_sel,
            promo=bool(promo),
            descuento=descuento,
...
@bp_bocadillos.route('/<nombre>/eliminar', methods=['GET', 'POST'])
def eliminar(nombre):
    ...
    if request.method == 'POST':
        try:
            ...
            flash(f"Bocadillo {nombre} eliminado.", "info")
    ...
    return render_template(
        ...
        nombre=nombre,
        url=url_for('bocadillos.listar')
    )

@bp_bocadillos.route('/api')
def api_bocadillos():
    ...
    for b in bocadillos:
        ...
        data.append({
            ...
            "ingredientes": [
                i.nombre for i in b.ingredientes
            ]
        })
...
@bp_bocadillos.route('/api/<nombre>')
def api_bocadillo(nombre):
    ...
    return jsonify({
        ...
        "autor": bocadillo.autor.nombre,
        "promocion": bocadillo.es_promocional(),
        "descuento": (
            bocadillo.descuento
            if bocadillo.es_promocional()
            else 0
