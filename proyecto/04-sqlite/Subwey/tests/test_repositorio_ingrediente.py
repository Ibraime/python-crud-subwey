...
class TestRepositorioIngrediente(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Repositorio usando conexión compartida
        self.repo = RepositorioIngrediente(conn=self.conn)
    ...
    def test_reponer_aumenta_stock(self):
        ...
        tomate_actualizado = self.repo.obtener_por_nombre("tomate")
        self.assertEqual(tomate_actualizado.stock, stock_inicial + 5)
    ...
    def test_listar_sin_elementos(self):
        self.conn.execute("DELETE FROM ingredientes")
        self.conn.commit()
